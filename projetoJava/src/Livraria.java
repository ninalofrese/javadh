    ...
    public void EfetuarVenda(Colecao colecao) {
        ...
        for (int i = 0; i < livros.size(); i++) {
            if (livros.get(i).getQuantidade() > 0) {
                ...
                System.out.println("Coleção: " + colecao.getDescricao());

                System.out.println("Livro vendido: " + livros.get(i).getCodigo());
                System.out.println("Estoque do livro: " + livros.get(i).getQuantidade());
            } else{
                System.out.println("Estoque da coleção esgotado.");
            }
            System.out.println();
        }
